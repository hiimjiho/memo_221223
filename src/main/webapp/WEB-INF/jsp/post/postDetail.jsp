	...
	<div class="w-50">
		...
		<div class="d-flex justify-content-between">
			...
			<div>
				...
				<button type="button" id="saveBtn" class="btn btn-success" data-post-id="${post.id}">수정</button>
			</div>
		</div>
	</div>
</div>
<script>
	$(document).ready(function(){
		...
		$("#saveBtn").on("click", function(){
			...
			// 파일이 업로드 된 경우 확장자 체크			
			if(file != ""){
				let ext = file.split(".").pop().toLowerCase();	// .pop은 마지막 배열을 가져오는 것,
				// alert(ext);
			...
			$.ajax({
				...
				, url: "/post/update"
				, data: formData
				, ecnType: "multipart/form-data"	// 파일 업로드를 위한 필수 설정
